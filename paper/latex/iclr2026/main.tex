...
allocation is near-exact; the allocation is interior on a synthetic corpus and, on a real multi-hop
QA benchmark with a frozen reader scored by EM/F1, the same compounding gap appears at a tight
budget ($+0.050$ F1) and shrinks as the budget loosens, though the precise real-data optimum
remains open. Fourth, a multi-view ``facet-lens'' escapes the wall in
...
compressor acts on the retriever's output, the realizable set of the pipeline is the intersection
of two subspace-constrained families, so $\ceff\le\min(c_{\mathrm{retrieve}},c_{\mathrm{compress}})$,
with strict inequality generically, since the retriever's discriminative subspace and the
compressor's reconstructive subspace need not align.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.49\linewidth]{figures/e1_capacity_wall.png}
\hfill
\includegraphics[width=0.49\linewidth]{figures/e8_limit.png}
\caption{\textbf{C1.} Left: the free-embedding retrieval wall---realizability jumps sharply once
$\dr$ clears a critical value that grows with corpus size $n_d$. Right: on LIMIT, three real
embedder families all fail the adversarial pattern at full dimension.}
\label{fig:c1}
\end{figure}
...
the compressor overflows, and re-allocating the same budget buys accuracy for free. Combined with
C1, which locates each wall cheaply by truncation, this is an actionable procedure: measure both
walls, then split the budget so retrieval is funded to $\dr^\star$ and the rest goes to compression.

\paragraph{Confirmation on a real multi-hop QA benchmark.} The synthetic pipeline of E6 isolates the
mechanism, but the claim that matters is whether it survives on an actual question-answering task
...
\texttt{Qwen2.5-1.5B-Instruct} answer, scored by EM/F1. At $B=128$ the best split ($\dr{=}96$) reaches
F1$=0.337$ against a standalone retrieval reference of $0.387$ and a standalone compression reference
of $0.564$---a compounding gap of $0.050$ F1 on real answer quality---and the gap shrinks to
$-0.011$ (effectively null) by $B=256$, the same qualitative shrinking-with-budget pattern as the
synthetic result. We report one scope limit honestly: F1 rises monotonically to $\dr{=}96$, the
edge of our tested grid, at both budgets, so unlike the synthetic case we have not yet located an
\emph{interior} turnover point for real retrieval budget on this task; we treat the compounding
result as confirmed on real data and the precise real-data optimum as still open.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.48\linewidth]{figures/c4_facetlens_escape.png}
\hfill
\includegraphics[width=0.48\linewidth]{figures/c4b_real_facetlens.png}
...
E9 & real & HotpotQA; Qwen2.5-1.5B; EM/F1 & gap $+0.050$ F1 at $B{=}128$; optimum not yet bracketed\\
